...
def magnitud(cuentas):
	return (-2.5*np.log10(cuentas)+25)
...
if parte_5 == True:
	...
	archivo_1.write('posx/ posy/ cuentas_g/ magnitud_g/ cielo_g/ cuentas_r/ magnitud_r/ cielo_r \n')
	for i in range(len(pos_estrellas_x)):
		...
		l=str(s)+'\t'+str(d)+'\t'+str(round(ya,3))+'\t'+str(ye)+'\t'+str(yi)+'\t'+str(round(ta,3))+'\t'+str(te)+'\t'+str(ti)+'\t\n'
		archivo_1.write(l)
